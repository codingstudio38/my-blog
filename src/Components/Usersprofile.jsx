...
import { USER_DETAILS, API_URL,USER_LOGOUT,Truncatetext ,WEBSITE_URL,decrypt,encrypt} from './Constant.jsx';
  ...
  const [musiccurrentpage, setmusiccurrentpage] = useState(1);
    ...
    useEffect(() => {
        ...
        if (firstCall.current) {
          ...
          userid = decrypt(decodeURIComponent(userid));
          if(userid==""){
            USER_LOGOUT();
            navigate('/');
            return;
 ...
 async function MyFriends() {
        try {
            ...
            setlistloader(true);
                ...
                let myform = JSON.stringify({user_id:userid,name:search_name});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
 ...
 async function MyVideolist() {
        try {
            ...
            setvideolistloader(true);
                ...
                let myform = JSON.stringify({user_id:userid,title:'',blog_type:'691beef0c2cfd41cc117ef6f'});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
     ...
     async function MyPhotolist() {
        try {
            ...
            setphotolistloader(true);
                ...
                let myform = JSON.stringify({user_id:userid,title:'',blog_type:'691beef0c2cfd41cc117ef70'});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
    ...
    async function MyReelslist() {
        try {
            ...
            setreellistloader(true);
                ...
                let myform = JSON.stringify({user_id:userid,title:'',blog_type:'691beef0c2cfd41cc117ef6e'});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
    ...
    async function MyMusiclist() {
        try {
            ...
            setmusiclistloader(true);
                ...
                let myform = JSON.stringify({user_id:userid,title:'',blog_type:'691beef0c2cfd41cc117ef71'});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
...
function viewprofile(user){
    let userid = encodeURIComponent(encrypt(user._id));
    navigate(`/web/users-profile/${userid}`);
    return true;
}
    ...
    return (
        <>
            <div className="container usersprofile">
 ...
 {photodatalist.map((item, index) => 
              ...
              <div className="fb-actions">
                        <button className={item.mylike > 0 ? 'fb-btn like active' : 'fb-btn like'} type='button' >
                            <i className="bi bi-hand-thumbs-up"></i> {item.total_likes} {item.total_likes <= 1 ? 'Like' : 'Likes'} 
                        </button>
                        <button className={item.mycomment > 0 ? 'fb-btn comment active' : 'fb-btn comment'} type='button' >
                            <i className="bi bi-chat"></i> {item.total_comments} {item.total_comments <= 1 ? 'Comment' : 'Comments'} 
                        </button>
                        <button className="fb-btn share " type='button' >
...
<Pagination pageSize={musiclimit} total={total_music_rec} current={musiccurrentpage} onChange={(value) => setmusiccurrentpage(value)} showQuickJumper={false} />
            ...
            <div className={c_tabe=='profile-friends' ? 'tab-pane fade active show' : 'tab-pane fade'} id="profile-friends">
                <div className="m-b-10 text-dark">
                <b>{total_friend_rec <=1? `${total_friend_rec} Friend` : `${total_friend_rec} Friends`}</b>
              </div>
                ...
                <li key={index}>
                  ...
                  >
                    ...
                    <div className="friend-info">
                      ...
                      <button type="button" className="btn btn-primary ml-4"  onClick={() => viewprofile(item)}>View Profile</button>
                    </div>
                  </a>
                </li>
 )}
