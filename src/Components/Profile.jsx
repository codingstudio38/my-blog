...
import { USER_DETAILS, API_URL,USER_LOGOUT,Truncatetext ,WEBSITE_URL,encrypt} from './Constant.jsx';
...
import swal from 'sweetalert';
 ...
 export default function Profile() {
  ...
  const [musiccurrentpage, setmusiccurrentpage] = useState(1);
    ...
    useEffect(() => {
        document.title = "MERN Technology || My Profile";
 ...
 async function MyVideolist() {
        try {
            ...
            setvideolistloader(true);
                ...
                let myform = JSON.stringify({user_id:LOGIN_USER._id,title:'',blog_type:'691beef0c2cfd41cc117ef6f',is_archive:0});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
     ...
     async function MyPhotolist() {
        try {
            ...
            setphotolistloader(true);
                ...
                let myform = JSON.stringify({user_id:LOGIN_USER._id,title:'',blog_type:'691beef0c2cfd41cc117ef70',is_archive:0});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
    ...
    async function MyReelslist() {
        try {
            ...
            setreellistloader(true);
                ...
                let myform = JSON.stringify({user_id:LOGIN_USER._id,title:'',blog_type:'691beef0c2cfd41cc117ef6e',is_archive:0});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
    ...
    async function MyMusiclist() {
        try {
            ...
            setmusiclistloader(true);
                ...
                let myform = JSON.stringify({user_id:LOGIN_USER._id,title:'',blog_type:'691beef0c2cfd41cc117ef71',is_archive:0});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
...
//     }
    ...
    let listtypeRef = useRef('');
    async function OpenComment(item,listtype) {
        try {
    ...
    const hideComments = (comment_) => {
        ...
        .then(async (willDelete) => {
            if (willDelete) {
                let url = `${API_URL}/hide-blog-comment`;
                let myform = JSON.stringify({comment_id:comment_._id,blog_id:comment_.blog_id,user_id:LOGIN_USER._id});
                let headers = {
                    'Content-Type': 'application/json',
                    'authorization': `Bearer ${LOGIN_USER.token}`,
                };
                ...
                <li key={index}>
                  ...
                  >
                    ...
                    <div className="friend-info">
                      ...
                      <button disabled={disabled_sendrequest?true : false} type="button" onClick={() => viewprofile(item)} className="btn btn-primary ml-4">View Profile</button>
                    </div>
                  </a>
                </li>
 )}
